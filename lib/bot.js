...
const REQUIRED_FIELDS = ['server', 'nickname', 'channelMapping', 'discordEmail', 'discordPassword', 'ircBroadcastUser'];
...
class Bot {
  constructor(options) {
    ...
    this.ircBroadcastUser = options.ircBroadcastUser;
  ...
  sendToDiscord(author, channel, text) {
    ...
    const ircBroadcastUser = this.invertedMapping[ircBroadcastUser];
    if (author == ircBroadcastUser) {
